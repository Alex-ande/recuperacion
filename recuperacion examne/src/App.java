...
public class App {
        public static void main(String[] args) throws Exception {
                ...
                // 1 - Ordenar por edad en orden descendente
                controller.ordenarPorEdadDesc(personas);
                System.out.println("Personas ordenadas por edad (descendente):");
                for (Persona p : personas) {
                    System.out.println(p.getNombre() + " - " + p.getEdad());
                }
        
                // 1.2 - Buscar personas por edad
                int[] edadesBuscar = { 25, 70 };
                for (int edad : edadesBuscar) {
                    int pos = controller.buscarPorEdad(personas, edad);
                    if (pos != -1) {
                        System.out.println("Encontrado: " + personas[pos].getNombre() + " - " + personas[pos].getEdad() + " en posición " + pos);
                    } else {
                        System.out.println("No se encontró una persona con edad: " + edad);
                    }
                }
        
                // 2 - Ordenar por nombre en orden ascendente
                controller.ordenarPorNombreAsc(personas);
                System.out.println("\nPersonas ordenadas por nombre (ascendente):");
                for (Persona p : personas) {
                    System.out.println(p.getNombre() + " - " + p.getEdad());
                }
        
                // 2.2 - Buscar personas por nombre
                String[] nombresBuscar = { "Anais", "Miguel" };
                for (String nombre : nombresBuscar) {
                    int pos = controller.buscarPorNombre(personas, nombre);
                    if (pos != -1) {
                        System.out.println("Encontrado: " + personas[pos].getNombre() + " - " + personas[pos].getEdad() + " en posición " + pos);
                    } else {
                        System.out.println("No se encontró una persona con nombre: " + nombre);
                    }
                }
            }
        }
